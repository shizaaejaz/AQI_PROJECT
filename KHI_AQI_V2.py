...
def get_forecast(df_capped, model_path="best_random_forest.pkl", save_csv=True):
    ...
    AQI min/max are based on last 10 days historical variation.
    
    Parameters:
    - df_capped: pd.DataFrame containing historical 5-hourly data with columns
                 ['timestamp','aqi','temperature','humidity','wind_speed', ...]
    ...
    # Average temp, humidity, wind_speed from last 3 days
    ...
    # Historical AQI range from last 10 days
    ten_days_ago = df_capped["timestamp"].max() - pd.Timedelta(days=10)
    recent_10days = df_capped[df_capped["timestamp"] > ten_days_ago]
    aqi_min_hist = recent_10days["aqi"].min()
    aqi_max_hist = recent_10days["aqi"].max()
    aqi_range = max(aqi_max_hist - aqi_min_hist, 1)  # avoid 0
    ...
    for i, date in enumerate(forecast_dates):
        ...
        predicted_aqi = rf_model.predict(X_future)[0]

        # Add small random variation (±10% of historical range)
        variation = np.random.uniform(-0.1, 0.1) * aqi_range
        predicted_aqi = max(0, predicted_aqi + variation)

        # Use last 10 days historical min/max to set aqi_min and aqi_max
        aqi_min = max(aqi_min_hist, predicted_aqi - 0.1 * aqi_range)
        aqi_max = min(aqi_max_hist, predicted_aqi + 0.1 * aqi_range)
...
# Upload new CSV
dataset_api.upload("latest_predictions.csv", "aqi_predictions",overwrite=True)
